...
// // ─── Types ────────────────────────────────────────────────────────────────────
// type LicenseStatus = 'VERIFIED' | 'PENDING' | 'SUSPENDED';
// type DocStatus = 'VERIFIED' | 'PENDING' | 'REJECTED';

// // ── Document shape — swap staticDocs[] with API response later ────────────────
// interface HospitalDocument {
//   id: string;
//   title: string;        // e.g. "Hospital Registration"
//   docType: string;      // e.g. "registration", "license", "safety"
//   status: DocStatus;
//   statusNote: string;   // e.g. "Verified on Oct 24, 2023"
//   icon: string;         // emoji
//   // Full document detail — fill from API later
//   docNumber?: string;
//   issuedBy?: string;
//   issuedTo?: string;
//   issuedDate?: string;
...
//   iconBg: string;
//   iconEmoji: string;
//   // Hospital detail fields — filled from API
//   legalName: string;
...
// // ─── Static fallback docs — replace with API response later ──────────────────
// // When your API returns documents for a hospital, map them to HospitalDocument[]
// // and pass into mapHospital. Currently using static placeholders.
// const STATIC_DOCS: HospitalDocument[] = [
//   {
//     id: 'doc-1',
//     title: 'Hospital Registration',
//     docType: 'registration',
//     status: 'VERIFIED',
//     statusNote: 'Verified on Oct 24, 2023',
//     icon: '🏥',
//     docNumber: '—',
//     issuedBy: '—',
//     issuedTo: '—',
//     issuedDate: '—',
//     expiryDate: '—',
//     description: 'Hospital registration document issued by the health authority.',
//     pages: 4,
//     fileSize: '1.2 MB',
//   },
//   {
...
//     docType: 'license',
//     status: 'PENDING',
//     statusNote: 'Awaiting secondary review',
//     icon: '📋',
//     docNumber: '—',
//     issuedBy: '—',
//     issuedTo: '—',
//     issuedDate: '—',
//     expiryDate: '—',
//     description: 'Medical license pending verification.',
//     pages: 2,
//     fileSize: '540 KB',
//   },
//   {
...
//     docType: 'safety',
//     status: 'REJECTED',
//     statusNote: 'Document expired on Sep 2023',
//     icon: '🔥',
//     docNumber: '—',
//     issuedBy: '—',
//     issuedTo: '—',
//     issuedDate: '—',
//     expiryDate: '—',
//     description: 'Fire safety certificate — expired.',
//     pages: 1,
//     fileSize: '280 KB',
//   },
// ];

// // ─── Mapper — extend when API returns more fields ─────────────────────────────
// const parseStaff = (staff: string) => {
...
//   // ── To map real docs from API, replace STATIC_DOCS with:
//   // const documents = (h.documents ?? h.licenseDocuments ?? []).map(mapDoc);
//   const documents: HospitalDocument[] = STATIC_DOCS;
...
//   'All Statuses', 'VERIFIED', 'PENDING', 'SUSPENDED',
// ];
...
//   return (
//     <View style={pb.track}>
//       <View style={[pb.fill, { width: `${percent}%` as any }]} />
//     </View>
//   );
// }
...
// // ─── Document Viewer Modal (full doc preview) ─────────────────────────────────
...
//                 {doc?.description ? (
//                   <Text style={dv.description}>{doc.description}</Text>
//                 ) : null}
...
//   fileRow:      { flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: '#F8FAFC', borderRadius: 10, padding: 12, marginBottom: 8 },
//   fileIconBox:  { width: 36, height: 36, borderRadius: 8, backgroundColor: '#EFF6FF', alignItems: 'center', justifyContent: 'center' },
//   fileEmoji:    { fontSize: 16 },
//   fileName:     { fontSize: 12, fontWeight: '600', color: '#0F172A' },
//   fileMeta:     { fontSize: 11, color: '#94A3B8', marginTop: 2 },
//   viewBtn:      { backgroundColor: '#EFF6FF', borderRadius: 8, paddingHorizontal: 14, paddingVertical: 7 },
//   viewBtnTxt:   { fontSize: 12, color: '#2563EB', fontWeight: '700' },
...
// // Shows: hospital details + scrollable list of documents + tap doc → doc viewer
...
// function HospitalReviewModal({ visible, hospital, onClose }: HospitalReviewModalProps) {
...
//                       <Text style={rm.detailFieldValue}>{hospital.currentAddress}</Text>
//                     </View>
//                     <View style={rm.twoColCell}>
//                       <Text style={rm.detailFieldLabel}>LOCATION</Text>
//                       <Text style={rm.detailFieldValue}>{hospital.location}</Text>
//                     </View>
//                   </View>
...
//                       <Text style={rm.detailFieldValue}>{hospital.staffCount || `${hospital.totalStaff}`}</Text>
//                     </View>
//                   </View>
...
//               <TouchableOpacity style={dv.rejectBtn} onPress={() => setDecision('rejected')} activeOpacity={0.85}>
//                 <Text style={dv.rejectBtnTxt}>✕   Reject</Text>
//               </TouchableOpacity>
//               <TouchableOpacity style={dv.approveBtn} onPress={() => setDecision('approved')} activeOpacity={0.85}>
...
//       {/* ── Document Viewer (nested, opens on top) ── */}
//       <DocumentViewerModal
//         visible={docViewerVisible}
//         doc={selectedDoc}
//         hospitalName={hospital.legalName}
//         onClose={() => {
...
//   overlay: { flex: 1, backgroundColor: 'rgba(15,23,42,0.45)', justifyContent: 'center',alignItems: 'center' },
//   sheet:   {
//     backgroundColor: '#fff', borderRadius: 16,width: '50%',
...
//     borderRadius: 12,
//     borderWidth: 1,
//     borderColor: '#5c81a7', // Very subtle border to match the image
//     padding: 20,
//     marginBottom: 10,
//   },
//   infoRow: {
...
//   },
//   infoRowLast: {
//     marginBottom: 0, // No bottom margin for the last item
//   },
//   detailFieldLabel: {
//     fontSize: 10,
//     fontWeight: '700',
//     color: '#64748B', // Muted text color from the image
//     textTransform: 'uppercase',
//     letterSpacing: 1,
//     marginBottom: 8,
//   },
//   detailFieldValueLegal: {
//     fontSize: 18, // Slightly larger for the hospital name
//     fontWeight: '800',
//     color: '#0F172A',
//   },
//   detailFieldValue: {
//     fontSize: 15,
//     fontWeight: '500', // Slightly lighter font weight for address/location
//     color: '#334155',
//     lineHeight: 22,
//   },
//   twoColRow: {
...
//     marginBottom: 24, // Spacing before the next row
//   },
//   twoColCell: {
//     flex: 1,
//     paddingRight: 12, // Prevents text from overlapping between columns
//   },
//   staffCountRow: {
//     flexDirection: 'row',
//     alignItems: 'center',
...
//   staffIcon: {
//     fontSize: 14,
//     color: '#047857', // Matching the dark green icon tint in the image
//   },
// });
...
// }
// function ActionMenu({ visible, onClose, onReview, onVerify, onReject, anchorY, anchorX }: ActionMenuProps) {
...
//         <Text style={hr.staffNum}>{h.totalStaff}</Text>
...
//   staffNum:  { fontSize: 22, fontWeight: '800', color: '#0F172A', textAlign: 'center' },
...
//         <SkeletonBox width={32} height={22} style={{ borderRadius: 6 }} />
...
//       const mapped: Hospital[] = (res?.data ?? []).map(mapHospital);
//       setHospitals(mapped);
//     } catch {
...
//       const matchStatus = status === 'All Statuses' || h.licenseStatus === status;
//       const matchCity   = city === 'All Cities' || h.city?.toLowerCase().includes(city.toLowerCase());
//       return matchSearch && matchStatus && matchCity;
//     });
//   }, [hospitals, search, status, city]);

//   const handleApply = () => { setSearch(searchDraft); setStatus(statusDraft); setCity(cityDraft); };
//   const handleClear = () => {
...
//               <TouchableOpacity onPress={() => { setSearch(''); setSearchDraft(''); }} hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}>
...
//               <TouchableOpacity onPress={() => { setCity('All Cities'); setCityDraft('All Cities'); }} hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}>
//                 <Text style={s.chipX}>✕</Text>
//               </TouchableOpacity>
//             </View>
//           )}
...
//       {/* ── Pagination ── */}
//       <View style={s.footer}>
//         <Text style={s.footerTxt}>Showing {filtered.length} of {hospitals.length} hospitals</Text>
//         <View style={s.navRow}>
//           <TouchableOpacity style={s.navBtn}><Text style={s.navTxt}>‹</Text></TouchableOpacity>
//           <TouchableOpacity style={[s.navBtn, s.navActive]}><Text style={[s.navTxt, { color: '#2563EB' }]}>›</Text></TouchableOpacity>
//         </View>
//       </View>
...
//         onVerify={async () => {
//           try {
//             await adminAPI.verifyDocument(activeHospital?.id);
//             showToast(`${activeHospital?.name} verified`, 'success');
//             fetchHospitals();
//           } catch { showToast('Verification failed', 'error'); }
//         }}
//         onReject={async () => {
//           try {
//             await adminAPI.rejectDocument(activeHospital?.id, 'Invalid details');
//             showToast(`${activeHospital?.name} rejected`, 'error');
//             fetchHospitals();
//           } catch { showToast('Rejection failed', 'error'); }
//         }}
//       />

//       {/* ── Hospital Review Modal (details + docs) ── */}
//       <HospitalReviewModal
...
// ─── Types ────────────────────────────────────────────────────────────────────
type LicenseStatus = 'VERIFIED' | 'PENDING' | 'SUSPENDED' | 'REJECTED';
type DocStatus = 'VERIFIED' | 'PENDING' | 'REJECTED';
...
// ─── Constants ────────────────────────────────────────────────────────────────
const ALL_STATUSES: Array<'All Statuses' | LicenseStatus> = [
  'All Statuses', 'VERIFIED', 'PENDING', 'REJECTED', 'SUSPENDED',
];

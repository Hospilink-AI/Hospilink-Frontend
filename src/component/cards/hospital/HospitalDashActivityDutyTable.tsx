...
function getStaffName(duty: ApiDuty): string {
  ...
  if (typeof duty.assignedTo === 'object') {
    return (duty.assignedTo as any).name ?? 'Assigned';
  }
  return 'Assigned';
}
...
// ─── Duty Row ─────────────────────────────────────────────
function DutyRow({ item, onMenuPress, onPress }: { item: ApiDuty; onMenuPress: () => void; onPress: () => void; }) {
    ...
    useCallback(() => {
      ...
      (async () => {
        try {
          ...
          // const list: ApiDuty[] = (res.data ?? []).map((d: any) => ({
          //   ...d,
          //   _id: d.dutyId,  
          //   assignedTo: d.staff,
          // }));
          ...
          const list: ApiDuty[] = Array.isArray(res.data)
            ? res.data.map((d: any) => ({
              ...d,
              _id: d.dutyId,
              assignedTo: d.staff,

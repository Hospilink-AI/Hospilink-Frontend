...
const getGreetingIcon = (): React.ComponentProps<typeof Ionicons>["name"] => {
  const h = new Date().getHours();
  if (h < 12) return "partly-sunny-outline";
  if (h < 17) return "sunny-outline";
  return "moon-outline";
};
...
export default function Header() {
  ...
  return (
    <View style={styles.container}>
      ...
      {/* ── RIGHT: greeting + fixed icons (flex:1 fills remaining space) ── */}
      <View style={styles.right}>

        {/*
          greetBlock has flexShrink:1 + minWidth:0
          → it compresses first before any icon is squished
          numberOfLines={1} + ellipsizeMode="tail" truncates long names with "…"
        */}
        <View style={styles.greetBlock}>
        ...
        {/* Time-of-day icon — flexShrink:0 so it never gets squished */}
        <Ionicons
...
const styles = StyleSheet.create({
  ...
  // ── RIGHT ─────────────────────────────────────────────
  right: {
    flex: 1,                     // absorbs all space left after logo
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "flex-end",  // icons stay pinned to the right
    gap: 8,
    minWidth: 0,                 // lets flex children shrink below intrinsic width
  },

  // Greeting block — only thing allowed to compress
  greetBlock: {
    flexShrink: 1,   // shrinks before any icon
    minWidth: 0,     // required for Text truncation in a flex child
    alignItems: "flex-end",
  },
  greet: {
    fontSize: 11,
  ...
  name: {
    ...
    fontWeight: "700",
    color: COLORS.text,
    // ellipsizeMode="tail" prop on the Text element handles the "…"
  },

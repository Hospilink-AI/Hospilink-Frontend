...
// // ⚠️ These values must match backend exactly: "2-10" | "11-50" | "51-100" | "100+"
// const STAFF_OPTIONS = [
...
//   // ✅ MOBILE VERSION (UNCHANGED - CORRECT)
//   if (!WebView) {
//     return (
...
//       onMessage={(e: any) => {
//         try {
//           const { lat: newLat, lng: newLng } = JSON.parse(
//             e.nativeEvent.data
//           );
...
//   const [staffCount, setStaffCount] = useState(STAFF_OPTIONS[2]); // default: "51-100"
//   const [showStaffDropdown, setShowStaffDropdown] = useState(false);
//   const [showServiceDropdown, setShowServiceDropdown] = useState(false);
//   const [selectedServices, setSelectedServices] = useState([
//     "Orthopedics", "Pediatrics", "Cardiology", "Emergency Care",
//   ]);
...
//   const handleStateChange = (val: string) => { setState(val); geocode(address, city, val); };

//   // ── Reverse geocode when pin is dragged/tapped
...
//   // body: { hospitalLegalName, currentAddress, servicesAvailable, location, staffCount }
//   // ────────────────────────────────────────────────────────────────────────
//   const handleFinishSetup = async () => {
//     // ── Validation
//     if (!hospitalName.trim()) {
//       showAlert("Missing Field", "Please enter the hospital legal name.");
//       return;
//     }
//     if (!address.trim()) {
//       showAlert("Missing Field", "Please enter the current address.");
//       return;
//     }
//     if (!city.trim()) {
...
//       showAlert("Missing Field", "Please select at least one service.");
//       return;
//     }

//     // Build full address string:  "address, city, state"
//     const fullAddress = [address.trim(), city.trim(), state.trim()]
//       .filter(Boolean)
//       .join(", ");

//     const payload = {
//       hospitalLegalName: hospitalName.trim(),   // "Deshmane Hospital"
//       currentAddress: fullAddress,            // "Near Lokmanya Tilak Road, Nagpur, Maharashtra"
//       servicesAvailable: selectedServices,       // ["General Surgery", "Emergency Care"]
//       location: city.trim(),            // "Nagpur"  ← backend uses city as location
//       staffCount: staffCount.value,          // "2-10" | "11-50" | "51-100" | "100+"
//     };
...
//       router.replace("/hospital/dashboard"); // ← navigate on success
//     } catch (error: any) {
...
//   progressFill: { width: "80%", height: "100%", backgroundColor: "#2563eb", borderRadius: 10 },
...
//   const [address, setAddress] = useState("");
//   const [city, setCity] = useState("");
//   const [state, setState] = useState("");             // ← now set via dropdown
//   const [pincode, setPincode] = useState("");         // ← added
//   const [staffCount, setStaffCount] = useState(STAFF_OPTIONS[2]);
//   const [showStaffDropdown, setShowStaffDropdown] = useState(false);
//   const [showStateDropdown, setShowStateDropdown] = useState(false); // ← added
//   const [showServiceDropdown, setShowServiceDropdown] = useState(false);
//   const [selectedServices, setSelectedServices] = useState<string[]>([]); // ← no pre-selection
//   const [loading, setLoading] = useState(false);
...
//   const handleAddressChange = (val: string) => { setAddress(val); geocode(val, city, state); };
//   const handleCityChange = (val: string) => { setCity(val); geocode(address, val, state); };
//   const handleStateSelect = (val: string) => {           // ← dropdown select
//     setState(val);
...
//   addServiceText: { color: "#64748b", fontSize: 12, fontWeight: "500" },
//   servicesHint: { color: "#94a3b8", fontSize: 12, fontStyle: "italic", marginTop: 2 },
//   servicesEmptyHint: { color: "#94a3b8", fontSize: 12, fontStyle: "italic" }, // ← added

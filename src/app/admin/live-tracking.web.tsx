    ...
    adminAPI.getHospitalsList()
      .then((res: any) => {
        if (res.success && res.data.length > 0) {
          ...
          setHospitalsError(res.message ?? 'No hospitals found.');
        }
      })
      .catch((err: any) => {
        const msg =
    ...
    adminAPI.getNearbyStaff(hospitalId, selectedDistance.value, selectedRole.value)
      .then((res: any) => {
        if (res.success) {
          ...
          setStaffError(res.message ?? 'Failed to load nearby staff.');
        }
      })
      .catch((err: any) => {
        const msg =
          err?.response?.data?.message ??
          err?.message ??
          'Failed to load nearby staff. Please try again.';
        setStaffError(msg);
      })
              ...
              onPress={() => {
                ...
                adminAPI.getHospitalsList()
                  .then((res: any) => {
                    ...
                    else setHospitalsError(res.message ?? 'Failed to load hospitals.');
                  })
                  .catch((err: any) => setHospitalsError(err?.response?.data?.message ?? 'Failed to load hospitals.'));
              }}
              style={{ backgroundColor: '#DC2626', paddingHorizontal: 14, paddingVertical: 6, borderRadius: 6 }}
            >
          ...
          <>
            ...
            {!loading && staffError ? (
              <View style={[styles.loadingOverlay, { backgroundColor: 'rgba(255,255,255,0.95)' }]}>
                ...
                <Text style={{ fontSize: 13, color: '#94A3B8', textAlign: 'center', marginBottom: 16, paddingHorizontal: 24 }}>
                  {staffError}
                </Text>
                <TouchableOpacity
                  onPress={() => {
                    ...
                    adminAPI.getNearbyStaff(hospitalId, selectedDistance.value, selectedRole.value)
                      .then((res: any) => {
                        if (res.success) {
                          setHospitalCoords({ latitude: res.data.hospital.location.latitude, longitude: res.data.hospital.location.longitude });
                          setStaff(jitterDuplicates(res.data.staff || []));
                        } else {
                          setStaffError(res.message ?? 'Failed to load nearby staff.');
                        }
                      })
                      .catch((err: any) => setStaffError(err?.response?.data?.message ?? 'Failed to load nearby staff.'))
                      .finally(() => setLoading(false));
                  }}
                  style={{ backgroundColor: '#3B82F6', paddingHorizontal: 24, paddingVertical: 10, borderRadius: 8 }}
                >

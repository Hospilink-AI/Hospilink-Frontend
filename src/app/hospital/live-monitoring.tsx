...
//             {/* ── Export Button ── */}
//             <TouchableOpacity style={styles.exportBtn} activeOpacity={0.8} onPress={exportActiveDuties}>
//               <Ionicons name="download-outline" size={16} color="#fff" />
//               <Text style={styles.exportBtnText}>Export Report</Text>
//             </TouchableOpacity>
//           </View>
...
// ─── Filter Select (reusable dropdown) ──────────────────────────────────────────
const FilterSelect = ({
  label,
  value,
  placeholder,
  ...
  return (
    <View style={[styles.filterWrap, open && { zIndex: 1000 }]}>
      <Text style={styles.filterLabel}>{label}</Text>
      <TouchableOpacity style={styles.selectBox} activeOpacity={0.8} onPress={onToggle}>
...
export default function LiveTrackingScreen() {
  ...
  // ── Fetch (server-side filtering via params) ──
  const fetchActiveDuties = useCallback(async () => {
    try {
      setError(null);
      ...
      const data: ActiveDutiesResponse = await dutyAPI.getHospitalActiveDuties({params});
      if (data.success) {
        setDuties(data.data);
        setSummary(data.summary);
      } else {
  ...
  // Mount + refetch on filter change. First run = full loader, later runs = light refetch.
  useEffect(() => {
    (async () => {
      if (!didMount.current) {
        setLoading(true);
    ...
    <View style={styles.screen}>
      ...
      >
        ...
        <View style={[styles.headerContainer, { zIndex: 10 }]}>
          ...
          <View style={styles.filtersRow}>
            ...
            <FilterSelect
              ...
              onToggle={() => setOpenFilter((p) => (p === 'status' ? null : 'status'))}
              onSelect={(v) => {
                setSelectedStatus(v);
                setOpenFilter(null);
              }}
        ...
        <View style={styles.subHeaderContainer}>
          ...
          {filtering && <ActivityIndicator size="small" color="#2563EB" />}
          {summary && (
            <View style={styles.summaryRow}>
              {summary.assignedCount > 0 && (
                <View style={[styles.summaryChip, { backgroundColor: '#ECFDF5' }]}>
...
const styles = StyleSheet.create({
  ...
  selectBox: {
    ...
    gap: 8, minWidth: 160, backgroundColor: '#fff',
    borderWidth: 1, borderColor: '#E5E7EB',
    paddingHorizontal: 14, paddingVertical: 11, borderRadius: 8,
  },
  selectText: { fontSize: 14, color: '#374151', fontWeight: '500', flexShrink: 1 },
  selectDropdown: {
    position: 'absolute',
    top: 70,
    left: 0,
  ...
  selectOption: { paddingVertical: 11, paddingHorizontal: 14 },
  selectOptionActive: { backgroundColor: '#EFF6FF' },
  selectOptionText: { fontSize: 14, color: '#374151' },
  selectOptionTextActive: { color: '#2563EB', fontWeight: '700' },

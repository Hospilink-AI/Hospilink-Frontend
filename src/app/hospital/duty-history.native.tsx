...
function ReviewModal({
  visible, duty, onClose, onSubmitted,
}: {
  visible: boolean; duty: Duty | null; onClose: () => void; onSubmitted?: () => void;
}) {
      ...
      <Pressable style={reviewStyles.overlay} onPress={onClose}>
        <Pressable style={reviewStyles.card} onPress={(e) => e.stopPropagation()}>
          ...
          <View style={reviewStyles.avatarWrap}>
            {profilePic ? (
              <Image source={{ uri: profilePic }} style={reviewStyles.avatarImg} resizeMode="cover" />
            ) : (
            ...
            {[1, 2, 3, 4, 5].map((star) => (
              Platform.OS === 'web' ? (
                ...
                <TouchableOpacity key={star} onPress={() => setRating(star)}>
                  <Text style={[reviewStyles.star, { color: star <= displayRating ? '#F59E0B' : '#E2E8F0' }]}>★</Text>
                </TouchableOpacity>
              )
            ))}
          </View>
...
const reviewStyles = StyleSheet.create({
  ...
  card: { backgroundColor: '#fff', borderRadius: 20, padding: 28, width: '100%', maxWidth: 400, alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 16 }, shadowOpacity: 0.2, shadowRadius: 32, elevation: 16 },
  ...
  avatarWrap: { width: 76, height: 76, borderRadius: 38, overflow: 'hidden', marginBottom: 12, borderWidth: 3, borderColor: '#E2E8F0' },
  ...
  textAreaWrap: { width: '100%', borderWidth: 1.5, borderColor: '#E2E8F0', borderRadius: 10, marginBottom: 18, backgroundColor: '#F8FAFC' },
  textArea: { minHeight: 80, padding: 12, fontSize: 13, color: '#1E293B' },
  submitBtn: { width: '100%', backgroundColor: '#2563EB', paddingVertical: 14, borderRadius: 10, alignItems: 'center', shadowColor: '#2563EB', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 5 },
  submitTxt: { color: '#fff', fontSize: 15, fontWeight: '700' },
  skipTxt: { fontSize: 13, color: '#94A3B8', textAlign: 'center' },
});

/* ================================================================
   ROLE PICKER BUTTON (mobile modal picker)
================================================================ */
...
function NativeSelect({
  ...
  value: string; options: { label: string; value: string }[];
  onChange: (v: string) => void; placeholder?: string;
  minWidth?: number; fullWidth?: boolean; isMobile?: boolean;
}) {
  if (Platform.OS === 'web') {
    if (isMobile) {
...
function DatePickerModal({
  ...
  visible: boolean; draft: DateFilter;
  onChange: (d: DateFilter) => void; onConfirm: () => void; onClose: () => void;
}) {
          ...
          <View style={{ marginBottom: 16 }}>
            {draft.mode === 'single' && (
              <View>
                ...
                {Platform.OS === 'web' ? (
                  <input type="date" value={draft.singleDate}
                    onChange={(e) => onChange({ ...draft, singleDate: e.target.value })}
                    style={webDateInput} />
                ) : (
                  <TextInput style={styles.nativeInput} placeholder="YYYY-MM-DD"
...
export default function DutyHistoryScreen() {
  ...
  const activeFilterCount = [
    !!(draftDate.singleDate || draftDate.startDate || draftDate.endDate),
    !!draftRole,
    !!draftStatus,
  ...
  const fetchDuties = useCallback(async () => {
    setLoading(true);
    try {
      const params: Record<string, any> = { page, limit: 10, ...buildDateParams(appliedDate) };
  ...
  const downloadReport = async () => {
    ...
    try {
      ...
      if (allDuties.length === 0) { Alert.alert('No data', 'No duties to export for the selected filters.'); return; }
      ...
      doc.setFillColor(37, 99, 235); doc.rect(0, 0, pageWidth, 35, 'F');
      doc.setTextColor(255, 255, 255); doc.setFontSize(20); doc.setFont('helvetica', 'bold');
      doc.text('Hospilink', 15, 20); doc.setFontSize(10); doc.setFont('helvetica', 'normal');
      ...
      autoTable(doc, {
        ...
        body: allDuties.map((d) => [
          formatDate(d.date), d.staff?.name || 'Unassigned', formatRole(d.staffRole),
          d.shiftDuration || '—', d.hoursCompleted || '—',
        ...
        columnStyles: { 0: { cellWidth: 22 }, 1: { cellWidth: 32 }, 2: { cellWidth: 32 }, 3: { cellWidth: 20 }, 4: { cellWidth: 20 }, 5: { cellWidth: 22 }, 6: { cellWidth: 32, halign: 'left' } },
      });
  ...
  const hasActiveFilter = !!(draftDate.singleDate || draftDate.startDate || draftDate.endDate || draftRole || draftStatus);
      ...
      <View style={styles.pageContainer}>
        ...
        <View style={styles.filterCard}>
          {isMobile ? (
            <View style={styles.filterCompactRow}>
              ...
              {/* ── Filter button + anchored dropdown ── */}
              <View style={{ position: 'relative', zIndex: 200 }}>
                <TouchableOpacity
                  style={styles.filterTriggerBtn}
                  onPress={() => setFilterSheetVisible((v) => !v)}
                  activeOpacity={0.75}
                >
                  <Text style={styles.filterTriggerIcon}>⚙</Text>
                  <Text style={styles.filterTriggerTxt}>Filters</Text>
                  {activeFilterCount > 0 && (
                ...
                {/* Dropdown anchored just below the button */}
                {filterSheetVisible && (
                  <>
                    ...
                    <View style={styles.filterDropdown}>
                      ...
                      <Text style={[styles.filterLabel, { marginTop: 10 }]}>STAFF ROLE</Text>
                      <NativeSelect
                        value={draftRole}
                      ...
                      <View style={styles.dropdownActions}>
                        {hasActiveFilter && (
                          <TouchableOpacity
                            ...
                            onPress={() => { handleClear(); setFilterSheetVisible(false); }}
                          >
                            <Text style={styles.sheetClearTxt}>Clear</Text>
                          </TouchableOpacity>
                        )}
                        <TouchableOpacity
                          style={[styles.sheetApplyBtn, !hasActiveFilter && { flex: 1 }]}
                          onPress={() => { handleApply(); setFilterSheetVisible(false); }}
                        >
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ flex: 1 }}>
                <View style={styles.filterChips}>
                  {(draftDate.singleDate || draftDate.startDate) && (
                  ...
                  {draftRole && (
                    <View style={styles.filterChip}>
                      <Text style={styles.filterChipTxt} numberOfLines={1}>{formatRole(draftRole)}</Text>
                    </View>
                  )}
                  {draftStatus && (
                    <View style={styles.filterChip}>
                      <Text style={styles.filterChipTxt}>{draftStatus}</Text>
                    </View>
                  )}
                </View>
              </ScrollView>
            ...
            <View style={styles.filterRowDesktop}>
              ...
              <View style={[styles.filterCol, { flex: 2.2 }]}>
                ...
                <NativeSelect value={draftRole} options={ROLE_OPTIONS} onChange={setDraftRole} placeholder="All Roles" isMobile={false} />
              </View>
              <View style={[styles.filterCol, { flex: 1.4 }]}>
                <Text style={styles.filterLabel}>STATUS</Text>
                <NativeSelect value={draftStatus} options={STATUS_OPTIONS} onChange={setDraftStatus} placeholder="All Statuses" isMobile={false} />
              </View>
        ...
        <View style={styles.listArea}>
          {loading ? (
            ...
            >
              {duties.map((d) => (
                <MobileCard key={d.dutyId} duty={d} onPress={() => handleRowPress(d)} />
              ))}
            </ScrollView>
          ) : (
            <View style={[styles.tableCard, styles.tableCardFill]}>
              ...
              <ScrollView style={styles.tableScroll} contentContainerStyle={styles.tableRowsContent} showsVerticalScrollIndicator>
                {duties.map((d, i) => (
                  <TableRow key={d.dutyId} duty={d} rowIndex={i} onPress={() => handleRowPress(d)} />
                ))}
              </ScrollView>
            </View>
          )}
    ...
    >
      <View style={[styles.tdFlex, COL.name]}>
        ...
        <View style={{ flex: 1, minWidth: 0 }}>
          ...
          {duty.staff?.email && <Text style={styles.staffEmail} numberOfLines={1}>{duty.staff.email}</Text>}
        </View>
      </View>
      ...
      <View style={[COL.hours, styles.tdCenter]}>
        <Text style={[styles.tdPrimary, { fontWeight: '700', color: '#0F172A' }]}>{duty.hoursCompleted || '—'}</Text>
      </View>
      <View style={[COL.status, styles.tdCenter]}>
        <View style={[styles.badge, { backgroundColor: sc.bg }]}>
          <View style={[styles.badgeDot, { backgroundColor: sc.dot }]} />
          <Text style={[styles.badgeTxt, { color: sc.text }]}>{duty.status ? duty.status.toUpperCase() : '—'}</Text>
        </View>
      </View>
      <View style={[COL.rating, styles.tdCenter]}>
        <StarRating value={rating} />
        {isClickable && <Text style={styles.rateHint}>Tap to rate</Text>}
      </View>
    </TouchableOpacity>
  );
}
  ...
  return (
    ...
    >
      ...
      <View style={styles.mGrid}>
        ...
        <MobileField label="Payment" value={`₹${(duty.totalPayment ?? 0).toLocaleString('en-IN')}`} valueColor="#2563EB" bold />
...
function MobileField({ label, value, bold, valueColor }: {
  label: string; value: string; bold?: boolean; valueColor?: string;
}) {
  return (
    <View style={styles.mField}>
      <Text style={styles.mFieldLabel}>{label}</Text>
      <Text style={[styles.mFieldValue, bold && { fontWeight: '700' }, valueColor ? { color: valueColor } : null]} numberOfLines={1}>
        {value}
      </Text>
    </View>
  );
...
/* ================================================================
   PAGINATION BAR  — compact for mobile
================================================================ */
      ...
      <View style={styles.pageControls}>
        ...
        {getPages().map((p, i) =>
          p === '...' ? (
            ...
            <PageBtn key={p} label={String(p)} onPress={() => onPage(p as number)} active={p === currentPage} />
          )
        )}
...
function PageBtn({ label, onPress, active, disabled }: {
...
const webDateInput: React.CSSProperties = {
  height: 40, width: '100%', paddingLeft: 12, paddingRight: 12,
  borderRadius: 8, border: '1.5px solid #E2E8F0',
...
const styles = StyleSheet.create({
  ...
  pageContainer: { flex: 1, padding: 16, minHeight: 0 },
  listArea: { flex: 1, minHeight: 0 },
  listScroll: { flex: 1, minHeight: 0 },
  listScrollContent: { gap: 10, paddingBottom: 8 },
  tableCardFill: { flex: 1, minHeight: 0 },
  tableScroll: { flex: 1, minHeight: 0 },
  tableRowsContent: { paddingBottom: 4 },

  /* ── Page Header ── */
  pageHeader: { flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 },
  pageTitle: { fontSize: 22, fontWeight: '700', color: '#0F172A', letterSpacing: -0.4 },
  pageSubtitle: { fontSize: 11, color: '#94A3B8', marginTop: 2 },
  exportBtn: { flexDirection: 'row', alignItems: 'center', borderWidth: 1.5, borderColor: '#CBD5E1', borderRadius: 9, paddingHorizontal: 12, paddingVertical: 7, backgroundColor: '#fff' },
  ...
  filterCard: {
    ...
    marginBottom: 10,
    zIndex: 100,             // ← add
    overflow: 'visible',     // ← add (so dropdown isn't clipped)
  },
  dropdownBackdrop: {
    position: 'absolute',
    top: -500,
  ...
  filterDropdown: {
    position: 'absolute',
    top: 44,           // sits just below the button
    left: 0,
    width: 230,        // fixed — doesn't stretch full width
    backgroundColor: '#fff',
    borderRadius: 12,
    borderWidth: 1,
    borderColor: '#E2E8F0',
  ...
  filterCompactRow: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  filterTriggerBtn: { flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 12, paddingVertical: 8, borderRadius: 8, borderWidth: 1.5, borderColor: '#E2E8F0', backgroundColor: '#F8FAFC' },
  filterTriggerIcon: { fontSize: 13, color: '#64748B' },
  filterTriggerTxt: { fontSize: 13, fontWeight: '700', color: '#334155' },
  filterBadge: { backgroundColor: '#2563EB', borderRadius: 10, minWidth: 18, height: 18, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 4 },
  filterBadgeTxt: { color: '#fff', fontSize: 10, fontWeight: '800' },
  filterChips: { flexDirection: 'row', gap: 6, alignItems: 'center' },
  filterChip: { backgroundColor: '#EFF6FF', borderRadius: 20, paddingHorizontal: 10, paddingVertical: 4, borderWidth: 1, borderColor: '#BFDBFE' },
  filterChipTxt: { fontSize: 11, color: '#1D4ED8', fontWeight: '600' },

  /* ── Mobile filter bottom sheet ── */
  sheetOverlay: { flex: 1, backgroundColor: 'rgba(15,23,42,0.45)', justifyContent: 'flex-end' },
  sheetCard: { backgroundColor: '#fff', borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 20, paddingBottom: 32, maxHeight: '85%' },
  sheetHandle: { width: 36, height: 4, backgroundColor: '#CBD5E1', borderRadius: 2, alignSelf: 'center', marginBottom: 16 },
  ...
  sheetClearBtn: { flex: 1, paddingVertical: 12, borderRadius: 9, borderWidth: 1.5, borderColor: '#E2E8F0', alignItems: 'center' },
  sheetClearTxt: { fontSize: 14, fontWeight: '600', color: '#64748B' },
  sheetApplyBtn: { flex: 2, backgroundColor: '#2563EB', paddingVertical: 12, borderRadius: 9, alignItems: 'center', shadowColor: '#2563EB', shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.28, shadowRadius: 5, elevation: 3 },

  /* ── Desktop filter row ── */
  filterRowDesktop: { flexDirection: 'row', alignItems: 'flex-end', gap: 12 },
  filterCol: { flex: 1.6 },
  filterActions: { flexDirection: 'row', alignItems: 'flex-end', gap: 8, paddingBottom: 1 },
  filterLabel: { fontSize: 10, fontWeight: '700', color: '#94A3B8', letterSpacing: 0.8, textTransform: 'uppercase', marginBottom: 5 },

  dateBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderWidth: 1.5, borderColor: '#E2E8F0', borderRadius: 8, paddingHorizontal: 10, height: 40, backgroundColor: '#fff', minWidth: 150, gap: 6 },
  dateBtnFull: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderWidth: 1.5, borderColor: '#E2E8F0', borderRadius: 8, paddingHorizontal: 10, height: 40, backgroundColor: '#fff', width: '100%', gap: 6 },
  dateBtnTxt: { fontSize: 13, color: '#1E293B', flex: 1 },

  nativeInput: { height: 40, borderWidth: 1.5, borderColor: '#E2E8F0', borderRadius: 8, paddingHorizontal: 10, fontSize: 13, color: '#1E293B', backgroundColor: '#fff', minWidth: 120 },

  applyBtn: { backgroundColor: '#2563EB', paddingHorizontal: 18, paddingVertical: 10, borderRadius: 8, shadowColor: '#2563EB', shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.28, shadowRadius: 5, elevation: 3 },
  applyBtnTxt: { color: '#fff', fontSize: 13, fontWeight: '700' },

  clearIconBtn: { width: 36, height: 36, borderRadius: 8, borderWidth: 1.5, borderColor: '#E2E8F0', backgroundColor: '#F8FAFC', alignItems: 'center', justifyContent: 'center' },
  clearIconTxt: { fontSize: 15, color: '#64748B', fontWeight: '700', lineHeight: 18 },

  /* ── Loading / Empty ── */
  loadingBox: { alignItems: 'center', paddingVertical: 70, gap: 12 },
  loadingTxt: { color: '#94A3B8', fontSize: 14 },
  emptyBox: { alignItems: 'center', paddingVertical: 70, backgroundColor: '#fff', borderRadius: 12, borderWidth: 1, borderColor: '#E2E8F0' },
  ...
  tHead: { flexDirection: 'row', backgroundColor: '#F8FAFC', paddingVertical: 12, paddingHorizontal: 16, borderBottomWidth: 1.5, borderBottomColor: '#E2E8F0' },
  th: { fontSize: 10, fontWeight: '700', color: '#94A3B8', letterSpacing: 0.7 },
  tRow: { flexDirection: 'row', paddingVertical: 14, paddingHorizontal: 16, borderBottomWidth: 1, borderBottomColor: '#F1F5F9', alignItems: 'center', minHeight: 62 },
  tRowAlt: { backgroundColor: '#FAFBFD' },
  tRowClickable: { cursor: 'pointer' as any },
  rateHint: { fontSize: 9, color: '#2563EB', marginTop: 2, fontWeight: '600' },
  ...
  /* Status badge */
  badge: { flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 9, paddingVertical: 4, borderRadius: 20 },
  badgeDot: { width: 6, height: 6, borderRadius: 3 },
  badgeTxt: { fontSize: 10, fontWeight: '700', letterSpacing: 0.3 },

  /* Mobile Card */
  mCard: { backgroundColor: '#fff', borderRadius: 12, padding: 14, borderWidth: 1, borderColor: '#E2E8F0', shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.04, shadowRadius: 3, elevation: 1 },
  mCardClickable: { borderColor: '#BFDBFE' },
  ...
  mRatePrompt: { marginTop: 10, paddingTop: 8, borderTopWidth: 1, borderTopColor: '#EFF6FF', alignItems: 'center' },
  mRatePromptTxt: { fontSize: 12, color: '#2563EB', fontWeight: '600' },

  /* Pagination — compact */
  pageBar: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 10, flexWrap: 'wrap', gap: 6 },
  pageInfo: { fontSize: 11, color: '#94A3B8' },
  pageControls: { flexDirection: 'row', alignItems: 'center', gap: 3 },
  pBtn: { minWidth: 30, height: 30, borderRadius: 6, borderWidth: 1.5, borderColor: '#E2E8F0', alignItems: 'center', justifyContent: 'center', backgroundColor: '#fff', paddingHorizontal: 3 },
  pBtnActive: { backgroundColor: '#2563EB', borderColor: '#2563EB' },
  pBtnOff: { backgroundColor: '#F8FAFC', borderColor: '#F1F5F9' },
  pBtnTxt: { fontSize: 12, fontWeight: '600', color: '#374151' },
  pBtnTxtActive: { color: '#fff' },
  pBtnTxtOff: { color: '#CBD5E1' },
  pageDots: { fontSize: 13, color: '#94A3B8', paddingHorizontal: 1 },

  /* Date Modal */
  modalOverlay: { flex: 1, backgroundColor: 'rgba(15,23,42,0.45)', justifyContent: 'flex-end', alignItems: 'center', padding: 20 },
  modalCard: { backgroundColor: '#fff', borderRadius: 16, padding: 24, width: '100%', maxWidth: 420, shadowColor: '#000', shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.18, shadowRadius: 24, elevation: 12 },
  modalTitle: { fontSize: 17, fontWeight: '800', color: '#0F172A', marginBottom: 16 },
  modalTabs: { flexDirection: 'row', gap: 8, marginBottom: 18 },
  modalTab: { flex: 1, paddingVertical: 9, borderRadius: 9, borderWidth: 1.5, borderColor: '#E2E8F0', alignItems: 'center', backgroundColor: '#F8FAFC' },
  ...
  modalFieldLabel: { fontSize: 11, fontWeight: '700', color: '#64748B', marginBottom: 6, letterSpacing: 0.3 },
  modalHint: { fontSize: 11, color: '#94A3B8', marginTop: 10, fontStyle: 'italic' },
  modalConfirmBtn: { backgroundColor: '#2563EB', borderRadius: 10, paddingVertical: 13, alignItems: 'center', marginTop: 4, shadowColor: '#2563EB', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 5 },
  modalConfirmTxt: { color: '#fff', fontSize: 15, fontWeight: '700' },

  /* Role picker modal */
  roleModalCard: { backgroundColor: '#fff', borderRadius: 10, padding: 20, width: '88%', position: 'absolute', bottom: 115 },
  roleModalHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 },
  roleOption: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 13, paddingHorizontal: 4, borderBottomWidth: 1, borderBottomColor: '#F1F5F9' },

...
// "2026-03-20" → "03/20/2026"  (for display)
...
function TimePickerField({ value, onChange, placeholder }: {
  ...
  const handlePress = () => {
    // Pre-fill picker with current value if present
    if (value) {
...
export default function CreateDutyScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Shape 1: { success: true, data: { ...duty } }
        // Shape 2: { ...duty }  (direct object)
        // Shape 3: { duty: { ...duty } }
        const d = res?.data ?? res?.duty ?? res;

        console.log('Duty fetched for edit:', d); // ← remove after confirming
        ...
        console.error('getDuty error:', err); // ← shows real error
        Alert.alert(
          'Error',
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEditMode && dutyId) {
        // ── EDIT ──────────────────────────────────────────
        await dutyAPI.updatePublishedDuty(dutyId, payload);  // PATCH /api/duties/:id
  ...
  // ── Loading state while fetching duty for edit ─────────
  if (loadingDuty) {
    return (
    ...
    <View style={styles.screen}>
      ...
      >
        ...
        <Text style={styles.pageTitle}>
          {isEditMode ? 'Edit Duty' : 'Create New Duty'}
        </Text>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.row2}>
            <View style={styles.col}>
              ...
              <DatePickerField value={form.startingDate} onChange={set('startingDate')} placeholder="mm/dd/yyyy" />
            </View>
            <View style={styles.col}>
              <FieldLabel label="Ending Date" />
              <DatePickerField value={form.endingDate} onChange={set('endingDate')} placeholder="mm/dd/yyyy" />
            </View>
          </View>
          ...
          {/* <View style={styles.row2}>
            <View style={styles.col}>
              ...
              <InputField placeholder="HH:MM  09:00" value={form.startTime} onChangeText={set('startTime')} />
            </View>
            <View style={styles.col}>
              <FieldLabel label="End Time" required />
              <InputField placeholder="HH:MM  17:00" value={form.endTime} onChangeText={set('endTime')} />
            </View>
          </View> */}
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: '#fff', borderRadius: 14, padding: 20, borderWidth: 1, borderColor: '#E5E7EB', marginBottom: 20 },
  ...
  dropdown: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 8, paddingHorizontal: 12, paddingVertical: 10, backgroundColor: '#fff' },
  ...
  inputWrap: { flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 8, paddingHorizontal: 12, backgroundColor: '#fff' },
  inputPrefix: { fontSize: 13, color: '#6B7280', marginRight: 4 },
  input: { flex: 1, fontSize: 13, color: '#111827', paddingVertical: Platform.OS === 'ios' ? 10 : 8 ,outlineWidth: 0,},
  ...
  overtimeBox: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#FFF4ED', borderRadius: 10, padding: 14, marginTop: 2 },
  ...
  actions: { flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'center', gap: 10, marginBottom: 16 },
  cancelBtn: { paddingHorizontal: 18, paddingVertical: 10 },
  cancelText: { fontSize: 14, color: '#6B7280', fontWeight: '500' },
  publishBtn: { paddingHorizontal: 20, paddingVertical: 10, borderRadius: 8, backgroundColor: '#2563EB', minWidth: 140, alignItems: 'center' },
  publishText: { fontSize: 14, color: '#fff', fontWeight: '700' },

  infoBox: { flexDirection: 'row', gap: 10, backgroundColor: '#EFF6FF', borderRadius: 10, padding: 14, alignItems: 'flex-start' },
